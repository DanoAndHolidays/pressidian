...
首页在 2026-09 做了一次整体改版：从"暖纸 + 柔和圆角 + 玻璃模糊"换成
**像素硬边 + CRT 信号故障**，并用三张巨构插画做背景。这一层是叠加的，
`main.css` 里的纸面设计系统没有被删掉——笔记正文、索引、关于页仍然用它。

| 文件 | 职责 |
| --- | --- |
| [`src/styles/pixel.css`](src/styles/pixel.css) | 像素设计系统：字体、硬边面板、色差、场景调色板、根画布 |
| [`src/components/scene/SceneStage.vue`](src/components/scene/SceneStage.vue) | 固定背景舞台：白天/黑夜两张画、视差、切换时的撕裂 |
| [`src/components/scene/GlitchLayer.vue`](src/components/scene/GlitchLayer.vue) | 全页故障：撕裂条、宏块丢失、扫描线扫过 |
| [`src/components/scene/PixelTypeLine.vue`](src/components/scene/PixelTypeLine.vue) | 逐字打字机（替代原首页的 `DecryptText`） |
| [`src/components/boot/BootSequence.vue`](src/components/boot/BootSequence.vue) | 启动动画：终端登录 → 拉幕 → 露出主页 |
| [`src/stores/scene.ts`](src/stores/scene.ts) | 昼夜循环状态（舞台与首页读数共用） |
| [`src/composables/useBootSequence.ts`](src/composables/useBootSequence.ts) | 启动阶段状态机 + 可取消的时间线 |
| [`src/composables/useGlitch.ts`](src/composables/useGlitch.ts) | 随机、不重复的故障爆发调度 |

#### 三条必须记住的约束

**1. 启动动画不是门禁，永远不要把它当门禁。**

站点是纯静态的 GitHub Pages 包，没有服务端可以校验任何东西。
"输入 Dano / 密码"是**自动播放的演出**，读者只需要按键跳过。
代码注释里写死了这一点：如果以后真要做访问控制，必须在
`BootSequence.vue` **之外**实现，不能改这个组件。

**2. `body` 不能有背景，否则会盖住固定舞台。**

这是改版时踩得最久的坑，值得单独说清楚：

`SceneStage` 是 `position: fixed; z-index: 0`，属于根层叠上下文。
根元素（`html`）绘制的背景落在 canvas 上，在舞台**下面**；
但 `body` 的 `background-attachment: fixed` 背景会画进根层叠上下文，
落在舞台**上面**。

结果是笔记索引页的半透明面板后面透出了夜景插画。
现在画布颜色由 `html` 拥有（`pixel.css`），纸面路由靠
`App.vue` 里的 `.px-app__canvas` 自己铺不透明底。
**不要往 `body` 上加 `background`，也不要加 `background-attachment`。**

**3. 像素字体只在 8px 的整数倍下清晰。**

Press Start 2P 是 8×8 点阵派生的，`--px-unit` 就是那个 8px。
`px-label` / `px-title` / `px-btn` 都用 `em` 从它派生。
随便写 `text-[0.72rem]`（11.5px）会让浏览器重采样每个字形，
整套"像素感"就糊掉了。改字号时用 `--px-unit` 的倍数。

两个字体都只有拉丁字形（`unicode-range` 显式声明了），
所以中文一律回落到系统字体——这是刻意的分工，
`px-label` 只用于拉丁标签、数字和英文小标题。

#### 场景素材

三张原图（1916×821 左右，各 2.4–3.3 MB）是 `design-source/scenes/` 下的母版，
**不参与部署**。`scripts/generate-scenes.mjs` 把它们转成
`public/scenes/*.{avif,webp}` 的两档宽度：

```bash
node scripts/generate-scenes.mjs
```

体积从约 9 MB 降到约 230 KB。[`src/data/scenes.ts`](src/data/scenes.ts)
读取生成出来的 `manifest.json` 拼 `<picture>` 的 srcset，
所以漏生成的分辨率会在构建期直接报错，而不是变成 404。

三张图各有职责，不是三选一：

- `day` / `night` —— 首页背景轮播的两端（14 秒一换）；
- `day-glitch` —— **只用于切换时的撕裂**，以及启动拉幕中间裂开的那道缝。

#### 启动动画的检视模式

拉幕最有意思的那一帧不到一秒，靠手速截不到。所以有：

```text
?intro=hold   停在幕布拉开 1/3 的位置不动
?intro=slow   整个序列 0.1 倍速
?intro=skip   直接跳到拉开之后
```

用 query 参数而不是构建开关，是为了在**已部署的 Pages 站点**上
也能直接检视，不用重新构建。

#### 昼夜调色板与主题开关是解耦的

`html[data-scene='day'|'night']` 覆盖的是一组 `--px-*` 变量，
和 `localStorage` 里的 `pressidian-theme`（纸面路由用的亮/暗）**不是一回事**。

原因：首页的界面是画上的装饰，背景画不能因为读者点了一下太阳图标就变。
页头的亮/暗按钮在非首页路由上照常工作。

#### 减少动态效果

`prefers-reduced-motion: reduce` 下：
启动动画整个跳过（直接进主页）、故障层 `display: none`、
视差和撕裂静止、打字机一次性给全文本。
不要为了"效果完整"绕过这个——自动播放且无法暂停的动画正是这条偏好要避免的。
